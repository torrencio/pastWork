...
XMLParser::XMLParser(string fileName)
{
	...
	{
		vector<string> tempAbilityArray;
		for(int i = 0; i < 12; i++){
			tempAbilityArray.push_back("");
		}
		...
		if(power_node->first_node("Weapon") != 0){
			if(power_node->first_node("Weapon")->first_node("Damage")->value() != 0)
				tempAbilityArray[10] = power_node->first_node("Weapon")->first_node("Damage")->value();
			if(power_node->first_node("Weapon")->first_node("AttackBonus")->value() != 0)
				tempAbilityArray[11] = power_node->first_node("Weapon")->first_node("AttackBonus")->value();
			/*
			cout << "here1" << endl;
			xml_node<> * damage_node = weapon_node->first_node("Damage");
			cout << "here2" << endl;
			tempAbilityArray[11] = weapon_node->first_node("AttackBonus")->value();
			cout << "here3" << endl;
			*/
		}
	...
	for(int i = 0; i < abilities.size(); i++){
		vector<string> temp;
		for(int j = 0; j < 12; j++){
			temp.push_back(abilities.at(i)[j]);
		}
		abil.push_back(temp);
	}
